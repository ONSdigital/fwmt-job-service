...
public class UserResourceServiceImplTest {
  ...
  public void cannotFindByAlternateAuthNo() {
    //Given
    String testAltAuth = "2222";
    when(restTemplate.exchange(any(),any(),any(),eq(UserDto.class),eq(testAltAuth))).thenThrow(new HttpClientErrorException(HttpStatus.BAD_REQUEST));
  ...
  public void altAuthNoUserNotActive() {
    ...
    String testAlthNo = "1111";
    Boolean isActive = false;
    when(restTemplate.exchange(any(),any(),any(),eq(UserDto.class),eq(testAlthNo))).thenThrow(new HttpClientErrorException(HttpStatus.BAD_REQUEST));
